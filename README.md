...
```
 ...
 [3] 일반화    (실험 15)
     ...
     └── starter/ 번들 + SSOT/config 분리 + 이식 시뮬레이션
              ↓ 이식은 됐다. 근데 설계 원칙은 여전히 사람에게만 있다.
 [4] 설계 내재화 (실험 21)
     ├── SOLID 같은 추상 원칙을 에이전트가 작업 시점부터 자동 따르게 할 수 있나?
     └── 원칙 → 프로젝트 맥락의 구체 규칙(R6~R9) → CLAUDE.md 등록 + 인프라 구축
```
...
실험 12·13·14·15·17·18·19·21 가 함께 만드는 **다층 방어**:

```
┌─────────────────────────────────────────────────────────────────────────┐
│ 설계 원칙 내재화 (SOLID 계층)                             ← 실험 21      │
│   CLAUDE.md R6~R9 + ESLint complexity/max-lines                         │
│   ┌──────────────────────────────────────────────────────────────────┐  │
│   │ R6 useQuery+useMutation 동시 사용 → lib/hooks/ 커스텀 훅 분리    │  │
│   │ R7 track() 컴포넌트 직접 호출 금지 → 훅 내부에서만              │  │
│   │ R8 req.json()+safeParse 직접 작성 금지 → withValidation()        │  │
│   │ R9 Number(id)+isFinite 직접 작성 금지 → withId()                 │  │
│   │ + ESLint complexity ≤10 / max-lines ≤200 자동 경고               │  │
│   │ → 에이전트가 새 코드를 쓸 때부터 설계 원칙을 따름               │  │
│   └──────────────────────────────────────────────────────────────────┘  │
└────────────────────────────────────┬────────────────────────────────────┘
                                     │ pass
                                     ▼
...
| SOLID·AOP 원칙이 사람의 머릿속에만 있고 에이전트에게 전달되지 않음 | 원칙 → 구체 규칙(R6~R9) → CLAUDE.md 등록 + `lib/hooks/` · `withValidation` · `withId` 인프라 + ESLint CC 신호 | 21 |
...
| i18n 키 (ko/en 평행) | **56** |
| 분석 이벤트 (PII 안전) | **23** |
| Hard 차단 규칙 | **5** (R1~R5, 코드베이스 false positive 0건 검증) |
| Soft 설계 규칙 | **4** (R6~R9, SOLID/AOP — CLAUDE.md 등록) |
| 자동 트리거 경로 | **4** (on-demand / pre-push / CI / 턴 종료 Stop hook) |
| 실험 결과 문서 | **21** (`results/01~21.md`) |
| `pr:gate` 실행 시간 | **~5초** |
...
│   ├── hooks/               # ← 실험 21: 커스텀 훅 (usePosts 등, SRP 분리)
│   └── api-route.ts         # ← 실험 21: withValidation / withId / withIdAndValidation
...
3. **설계 원칙까지 에이전트에 내재화할 수 있는가** — 실험 21 이 "방법은 있다" 에 수렴

가장 큰 자산은 테스트 83개도 Hard 규칙 5개도 아니다.
"**어디까지 Hard 로 강제하고, 어디부터 Soft 로 유도할지**"의 경계 문서 — 즉 `results/12~15.md` + `21.md` — 가 이 프로젝트의 진짜 산출물이다.
...
- **규칙 수 확장**: Hard 5 + Soft 4 = 9개에서는 관리 가능. 20개·50개로 갈 때의 유지비 곡선은 미측정.
- **Soft 규칙 재현성**: R6~R9 를 다른 모델·다른 세션에서도 일관되게 따르는지는 미측정. 에이전트가 무시해도 현재는 탐지 수단이 없음.

이 한계들은 시리즈의 **자연스러운 다음 실험** — 재현성 스모크, Soft 규칙 위반 탐지 자동화, 크로스-프레임워크 이식 — 으로 이어진다.
