...
`.husky/pre-push` 가 `git push` 직전에 Playwright E2E 를 강제 실행한다.

- 배경: exp 20 디자인 레이어가 헤딩 문자열을 바꿨는데 `pr:gate` 는 e2e 를 포함하지 않아 CI 에 가서야 셀렉터 파손이 드러났다. 이 훅은 "디자인/문구/라우트 변경이 통합 계층을 깼는지" 를 push 전에 잡는다.
- 이미 `npm run dev` 로 3000 이 떠 있으면 reuse 모드(`PORT=3000 npm run e2e`)로 돌아 Next 15 의 dev 싱글톤 충돌을 피한다.
- 긴급 우회: `git push --no-verify` 또는 `LAB_SKIP_E2E=1 git push`.
