...
- **새 이벤트는 `lib/analytics/events.ts`의 `EventMap`에 타입부터 추가**한다. 추가하지 않으면 `track()` 호출이 타입 에러로 즉시 막히며, 이 에러가 Skill의 자동 방어선이다.
...
- [ ] 새 analytics 이벤트 사용 시 `lib/analytics/events.ts::EventMap` 에 타입 먼저 추가 (TS 컴파일이 강제)

한 항목이라도 실패하면 **완료 보고 금지**. 수정 후 재검증.

## 레퍼런스 (반드시 참고)

- `components/CommentList.tsx` — 복잡한 폼 컴포넌트의 표준 패턴
- `components/SafeHtml.tsx` — 단순 래퍼의 표준 패턴
- `components/CommentList.stories.tsx` — decorator 주입 + 4케이스 구성
- `components/__tests__/SafeHtml.test.tsx` — 단위 테스트 스타일
- `lib/i18n/messages.ts` — 카탈로그 구조
